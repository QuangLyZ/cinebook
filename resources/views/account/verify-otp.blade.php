...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 flex justify-center">
    <div class="bg-gray-800 p-8 rounded-2xl border border-gray-700 shadow-xl w-full max-w-md mt-10">
        
        <div class="text-center mb-8">
            <h2 class="text-3xl font-extrabold text-white mb-2">Đổi Mật Khẩu</h2>
            <p class="text-gray-400 text-sm">Vui lòng nhập mã 6 số chúng tôi vừa gửi đến email của bạn để xác nhận đổi mật khẩu.</p>
        </div>
        ...
        <form action="{{ route('account.password.otp.verify') }}" method="POST" class="space-y-6">
            @csrf
            <div>
            ...
            <button type="submit" class="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-4 rounded-lg transition-colors flex justify-center items-center text-lg">
                <i class="fa-solid fa-check-circle mr-2"></i> Xác Nhận Đổi Mật Khẩu
            </button>
        </form>

        <div class="text-center mt-6">
            <p class="text-gray-400 text-sm">Chưa nhận được mã? <a href="{{ route('account.password.otp.resend') }}" class="text-blue-500 hover:text-blue-400 font-medium ml-1">Gửi lại OTP</a></p>
            <p class="text-gray-500 text-xs mt-4"><a href="{{ route('account.index', ['tab' => 'profile']) }}" class="hover:text-white transition">Hủy đổi mật khẩu</a></p>
        </div>
    </div>
</div>
@endsection
