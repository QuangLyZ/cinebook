...
<div class="max-w-2xl space-y-6 animate-[fadeIn_0.5s_ease-in-out]">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-extrabold tracking-tight text-white">Sửa Suất Chiếu</h2>
            <p class="mt-1 text-sm text-gray-400">Cập nhật thông tin cho suất chiếu #{{ $showtime->id }}.</p>
        </div>
    </div>
        ...
        <div class="rounded-3xl border border-gray-800 bg-gray-900/80 p-6 space-y-5">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                    ...
                    <select id="cinema_select"
                        ...
                        @foreach($cinemas as $cinema)
                            <option value="{{ $cinema->id }}"
                                    {{ $showtime->room?->cinema_id == $cinema->id ? 'selected' : '' }}>
                                {{ $cinema->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="room_id" id="room_select"
                            class="w-full rounded-xl border border-gray-700 bg-black/50 px-4 py-3 text-white focus:border-violet-500 focus:outline-none focus:ring-2 focus:ring-violet-500/20 appearance-none">
                        @foreach($cinemas as $cinema)
                            @if($showtime->room?->cinema_id == $cinema->id)
                                @foreach($cinema->rooms as $room)
                                    <option value="{{ $room->id }}"
                                            {{ old('room_id', $showtime->room_id) == $room->id ? 'selected' : '' }}>
                                        {{ $room->name }}{{ $room->seat_count ? ' (' . $room->seat_count . ' ghế)' : '' }}
                                    </option>
                                @endforeach
                            @endif
                        @endforeach
            ...
            <div>
                ...
                <select name="subtitle_id"
                    ...
                    @foreach($subtitles as $sub)
                        <option value="{{ $sub->id }}"
                                {{ old('subtitle_id', $showtime->subtitle_id) == $sub->id ? 'selected' : '' }}>
                            {{ $sub->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <p class="mt-2 text-xs text-gray-500">
                    ...
                    Hệ thống sẽ kiểm tra trùng lịch (bỏ qua suất chiếu hiện tại).
                </p>
            </div>
        </div>
...
@php
    ...
    $currentRoomId = $showtime->room_id;
@endphp
